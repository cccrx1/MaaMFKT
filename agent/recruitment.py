...
def _normalize_tag_box(box):
    x, y, w, h = [int(v) for v in list(box)]
    roi_x, roi_y, roi_w, roi_h = TAG_ROI

    # OCR is always called with roi=TAG_ROI (cropped image), so the returned
    # boxes are always in local coordinates. Convert to global 720x1280 space.
    x += roi_x
    y += roi_y

    # Expand the box slightly for easier clicking (tag buttons extend beyond text)
    x1 = max(roi_x, x - 24)
    y1 = max(roi_y, y - 16)
    x2 = min(roi_x + roi_w, x + w + 24)
    y2 = min(roi_y + roi_h, y + h + 16)
...
def _read_visible_tags(context, image):
    ...
    for result in _ocr_results(context, image, TAG_ROI):
        tag = _clean_tag(getattr(result, "text", ""))
        box = getattr(result, "box", None)
        if not tag or not box:
            continue
        if any(existing["tag"] == tag for existing in tags):
            continue
        tags.append({"tag": tag, "box": _normalize_tag_box(box)})
    return tags
