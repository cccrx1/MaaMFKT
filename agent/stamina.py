...
def _parse_stamina_text(text):
    ...
    numbers = re.findall(r"\d{1,4}", normalized)
    if not numbers:
        return None
    if len(numbers) == 1 and len(numbers[0]) >= 4:
        compact = numbers[0]
        if compact.endswith("150"):
            return int(compact[:-3])
        if compact.endswith("50") and len(compact) > 2:
            return int(compact[:-2])
        return int(compact[:-3] or compact[:-2])
